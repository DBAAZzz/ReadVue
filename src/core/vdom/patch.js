...
function sameVnode(a, b) {
  // key必须相同，需要注意的是 undefined === undefined => true
  return (
    a.key === b.key &&
...
export function createPatchFunction(backend) {
  ...
  /**
   * diff 过程：
   *    diff 优化：做了四种假设，假设新老节点开头结尾有相同节点的情况，一旦命中假设，就避免了一次循环，以提高效率
   *              如果不幸没有命中假设，则执行遍历，从老节点中找到新开始节点
   *              找到相同节点，则执行 patchVode，然后将老节点移动到正确的位置
  ...
  function updateChildren(parentElm, oldCh, newCh, insertedVnodeQueue, removeOnly) {
    ...
    while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
      if (isUndef(oldStartVnode)) {
        // 如果节点被移动，在当前索引上可能不存在，检测这种情况，如果不存在则调整索引
        ...
        // 老结束和新结束时同一个节点，执行 patch
        patchVnode(oldEndVnode, newEndVnode, insertedVnodeQueue, newCh, newEndIdx)
        // patch 结束后老结束和新结束的索引分别 -1
        oldEndVnode = oldCh[--oldEndIdx]
        ...
        // 如果上面的四种假设都不成立，则通过遍历找到新开始节点在老节点中的索引位置
    ...
    if (oldStartIdx > oldEndIdx) {
      // 说明老节点被遍历完了，新节点有剩余，则寿命这部分剩余的节点是新增的节点，然后添加这些节点
  ...
  function patchVnode(
    ...
    // 让 vnode.elm 引用到现在的真实 dom，当 elm 修改时，vnode.elm 会同步改变
    ...
    if (isUndef(vnode.text)) {
      ...
      // 新节点是文本节点，则更新文本节点
      // nodeOps 是一个适配层，根据不同平台提供不同的操作 DOM 的方法，实现跨平台
  ...
  return function patch(oldVnode, vnode, hydrating, removeOnly) {
    ...
    if (isUndef(oldVnode)) {
      ...
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        ...
        // 基于新 vnode 创建整颗 DOM 树并插入到 body 元素下
        createElm(
          vnode,
          insertedVnodeQueue,
